...
void MainFrame::on_tbtnUser_triggered(QAction *arg1)
{
    if(arg1 == zPersonalInfoAct){
        QString userInfo("\n用户名：King_N\n电话号码：12345678910\n职位：操作员\n车牌号：无\n");
        QMessageBox::about(this,"个人信息",userInfo);
    }
...
{
//    qDebug()<<"使用车位"<<usedStall;
//    qDebug()<<"剩余车位"<<restStall;
...
{
    auto ret = QMessageBox::question(this,tr("退出"),tr("你确定要离开伟大帅气风流倜傥聪明绝伦的开发者精心开发的这款强大的智能系统吗？"),
                                     tr("残忍离开"),tr("坚守岗位"),0);
    if(ret == 0){
        event->accept();
    }
