...
void zVideo::sloShowReadPic(QTcpSocket *tcpSocket)
{
    ...
    case 1:
        ...
        {
            tcpSocket->read(bufff, sizeof(bufff));
            qDebug()<<"数据:"<<bufff;
            QString data(bufff);
            data1=data;
            return ;
    ...
    case 2:
        ...
        pix->loadFromData((uchar *)pic_buf,pic_len, "BMP");
        pix->save("car1.bmp");          //将图片保存到当前路径
...
void zVideo::sloInNetworkPost()
{
    ...
    if(ui->rbtn_on1->isChecked()){
        ui->le_carNumber->setText(data1);
        ui->le_carNumber_2->setText(data1);
        QString msg="CP:J"+data1;
        udp->writeDatagram(msg.toUtf8().data(),30,QHostAddress("192.168.64.192"),6666);
        carUser user;
        sqlUser su;
        user = su.GetUserInfoByCarNum(data1);
...
void zVideo::sloOutNetworkPost()
{
    ...
    if(ui->rbtn_on2->isChecked() ){
        ui->le_carNumber_3->setText(data1);
        ui->le_carNumber->setText(data1);
        ...
        //添加数据库记录
        sqlCarState sc;
        carState car = sc.GetUserInfo(data1);
        car.State = tr("出库");
        ...
        QString msg(QString("金额:%1").arg(charge));
        QString msg1="CP:C"+data1;
        udp->writeDatagram(msg1.toUtf8().data(),30,QHostAddress("192.168.64.192"),6666);
        sleep(1);
        udp->writeDatagram(msg.toUtf8().data(),msg.length(),QHostAddress("192.168.64.192"),6666);
