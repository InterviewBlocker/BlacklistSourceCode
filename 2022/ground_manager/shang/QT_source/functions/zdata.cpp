...
void zData::on_pbtn_connect_clicked()
{
    ...
    {
        tcpClient::getInstance().connectTo(tcpSocket, QHostAddress("192.168.64.212"), 8888);
    }
...
void zData::on_btnRefreshCarUser_clicked()
{
    loadAllData(this->model->rowCount());
}

void zData::on_btnPay_clicked()
{
    Pay *pay = new Pay(this);
    pay->setWindowTitle("支付");
    pay->setWindowModality(Qt::WindowModal);
    pay->show();
}
